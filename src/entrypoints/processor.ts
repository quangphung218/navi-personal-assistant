...
import { openRouterFocusAssistant, openRouterStructuredAssistant } from '../adapters/openrouter';
  ...
  async queue(batch, env) {
    for (const message of batch.messages) {
      try {
        ...
        await processNext(env.DB, Date.now(), undefined,
          env.OPENROUTER_API_KEY ? openRouterStructuredAssistant(env.OPENROUTER_API_KEY) : undefined,
          env.OPENROUTER_API_KEY ? openRouterFocusAssistant(env.OPENROUTER_API_KEY) : undefined);
